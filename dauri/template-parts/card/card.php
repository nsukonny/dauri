<?php
/**
 * Detailed card of product
 *
 * @param WC_Product $wc_product Product object.
 */
$wc_product = $args['product'] ?? null;

if ( empty( $wc_product ) ) {
	return;
}

$product_discussion_url  = get_field( 'product_discussion_url', 'option' ) ?? '#';
$product_discussion_text = get_field( 'product_discussion_text', 'option' ) ?? __( 'ОБСУДИТЬ ДЕТАЛИ И ПРИОБРЕСТИ' );
$terms                   = get_the_terms( $wc_product->get_id(), 'product_brand' );
$brand                   = $terms[0]->name ?? '';

$attributes = array(
	'diameter'   => $wc_product->get_attribute( 'pa_diameter' ) ?: '',
	'components' => $wc_product->get_attribute( 'pa_components' ) ?: '',
	'condition'  => $wc_product->get_attribute( 'pa_condition' ) ?: '',
);

$attributes_html = mb_strtoupper( implode( ', ', $attributes ) );

$in_stock = $wc_product->is_in_stock() ? __( 'В НАЛИЧИИ' ) : __( 'ПО ЗАПРОСУ' );
?>
<section class="product-card">
    <div class="container">
        <div class="product-card-inner" data-aos="fade-in" data-aos-duration="3000">
            <div class="product-card-image">
				<?php echo $wc_product->get_image( 'full' ); ?>
            </div>
            <div class="product-card-status-wrapper">
                <button class="add-to-fav">
                    <svg xmlns="http://www.w3.org/2000/svg" width="39" height="39" viewBox="0 0 39 39" fill="none">
                        <path
                                d="M20.2707 10.4723C21.6419 8.98494 23.607 8.05469 25.7907 8.05469C29.86 8.05469 33.1731 11.2865 33.3057 15.3226C33.3074 15.384 33.3091 15.4453 33.3091 15.5067V15.5232C33.3007 15.8383 33.2593 16.5149 33.0653 17.4716C32.3838 20.8444 29.8252 25.0181 20.2689 31.8963C10.7343 25.0331 8.16569 20.8726 7.4792 17.4932C7.28187 16.5249 7.23875 15.84 7.23047 15.5232V15.5067C7.23047 15.4453 7.23213 15.384 7.23379 15.3226C7.36644 11.2865 10.6795 8.05469 14.7488 8.05469"
                                stroke="#1A1A1ACC" stroke-width="2.41692" stroke-miterlimit="10"
                                stroke-linecap="round"/>
                    </svg>
                </button>
                <div class="product-card-status">
					<span class="status-item">
						<img src="<?php echo esc_attr( THEME_URL ); ?>/assets/img/new.svg" alt="">
					</span>
                </div>
            </div>
            <div class="product-card-desc">
                <span class="product-card-model"><?php echo esc_html( $brand ); ?></span>
                <span class="product-card-name"><?php echo esc_html( $wc_product->get_name() ); ?></span>
                <p><?php echo esc_html( $attributes_html ); ?></p>
                <div class="indicator-wrapper">
                    <div class="product-card-indicator"><?php echo esc_html( $in_stock ); ?></div>
                </div>
                <span class="product-card-price">
					<?php echo $wc_product->get_price_html(); ?>
				</span>
            </div>
            <a class="button gradient full" href="<?php echo esc_attr( $product_discussion_url ); ?>" data-watch-button
               target="_blank">
				<?php echo esc_html( $product_discussion_text ); ?>
                <img src="<?php echo esc_attr( THEME_URL ); ?>/assets/img/whatsapp-w.svg" alt="">
            </a>
        </div>
    </div>
</section>